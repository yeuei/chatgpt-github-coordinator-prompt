...
- 用户决定项目目标、范围、正式验收标准和重大方向变化。
- ChatGPT Coordinator 负责入口选择、需求澄清、任务拆分、交接仓库当前事实、PR 协调和决策记录。
- GitHub 交接仓库保存当前项目事实、任务合同、事件、审批结果、任务快照和可审计历史。
- Codex Agent 负责本地代码、测试、运行、Git 镜像同步、执行报告和普通工程细节。
- Dashboard/Skill 负责运行时调度、健康检查和展示；不拥有项目协议或业务状态。

Agent 本地 Git 克隆是交接仓库的只读运行镜像，不是另一份项目真相。GitHub 是跨端交接事实来源；Dashboard 正常展示使用本地镜像，显式刷新才访问网络。
...
## 3.1 PR 与 commit 标题格式

创建或更新 PR 时，PR 标题和每个有意义的 commit subject 必须使用双语格式：`中文标题（English technical title）`。中文部分准确说明实际变更；英文部分用于技术检索且必须语义等价。禁止只写英文或使用与实际变更无关的泛化标题。PR 描述、任务和报告中的主要变更标题也应遵循同一格式；协议 trailer、分支名、代码符号和日志原文不需要翻译。
