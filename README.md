...
触发器可能处于全局暂停、单方向关闭或逐条人工审批状态。不要因未收到下一条唤醒而重复写同一事件、虚构失败或要求用户关闭安全门。用户也可以在本地 Dashboard 明确开启“自动审批模式”；此时本地服务会直接处理新事件和现有待审批事件，你不应再等待用户手动点击发送，但仍必须先以 GitHub 当前 HEAD 为准，且不要重复发送同一 event id（仅允许对本地明确标记为已验证 fill-only 草稿的事件补发一次）。
